...
string galvaMultiplication(string a,string b){
    string temp1 ;
    string res = "0000000000000000" ;
    for(int i = 0 ;i<b.size();i++){
        if(b[i] == '1')
        temp1 = leftShift(a,b.size()-i);
        res = XOR(res,temp1);
    }
    string galva = "10001101";
    string r = "";
    for(int i = 0;i<res.size()-7;i++){
        if(res[i] == '1'){
            r+='1';
            res[i] = XORCHAR('1',res[i]);
            res[i+4] = XORCHAR('1',r[i+4]);
            res[i+5] = XORCHAR('1',res[i+5]);
            res[i+7] = XORCHAR('1',res[i+7]);
        }
        else{
            r+='0';
        }
