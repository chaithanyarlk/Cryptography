...
void cipherBlockChainingEncryption(string plain,vector<vector<vector<string>>>&keys,string IV){
     string res = "";
     string init = IV;
     string tempb ="";
     ...
     int count  = 1;
     for(int i = 0;i<tempb.size();i=i+IV.size()){
          cout<<"Part of plain Text "<<count<<endl;count++;
          string temp = XOR(tempb.substr(i,128),init);
          string r ="";
...
int main(){
    ...
    cin.tie(0);
    cout.tie(0);
    string IV = "ABCDABCDABCD123456789123456789AB";
    IV = hextoBinHelper(IV);
    ...
    keyGenerator(keys);
    cipherBlockChainingEncryption(plain,keys,IV);
}