...
string cipherBlockModeEncryption(string str,vector<string>&roundkeysBin,vector<string>&roundkeysHex,int s,string IV){
     // s bits will be taken as a block 
     string res = "";
     string init  = IV;
     string plain = hextoBin(str);
     for(int i = 0;i<plain.size();i = i+s){
          string temp = plain.substr(i,s);
          string enc = encryptionAlgo(init,roundkeysBin,roundkeysHex);
          enc = hextoBin(enc);
          string last = XOR(temp,enc.substr(0,s));
          res += bintoHex(last);
          init = enc.substr(s,enc.size()-s)+last;
          init = bintoHex(init);
     } 
     return res;
}
string cipherBlockModeDecryption(string str,vector<string>&roundkeysBin,vector<string>&roundkeysHex,int s,string IV){
     string res = "";
     string init  = IV;
     string plain = hextoBin(str);
     for(int i = 0;i<plain.size();i = i+s){
          ...
          string last = XOR(temp,enc.substr(0,s));
          res += bintoHex(last);
          init = enc.substr(s,enc.size()-s)+temp;
          init = bintoHex(init);
     } 
     return res;
}
...
int main(){
    ...
    int s  = 32;
    IV = "9955ABCD11228855";
    plain = "9955ABCD112288559955ABCD11228855";
    key = "FEFEFEF123456789";
    ...
    cout<<"CFB Encryption Begins ..."<<endl;
    string cipher = cipherBlockModeEncryption(plain,roundkeysBin,roundkeysHex,s,IV);
    cout<<"Encrypted String ... "<<cipher<<endl;
    cout<<"CFB Decryption Begins ..."<<endl;
    string decrypt = cipherBlockModeDecryption(cipher,roundkeysBin,roundkeysHex,s,IV);
