...
string matriceToString(vector<vector<string>>s){
     string res = "";
     for(int i = 0;i<4;i++){
          for(int j = 0;j<4;j++){
               res += (s[j][i]);
...
void counterMode(string plain,vector<vector<vector<string>>>&keys,int count_val){
     bitset<128>b(count_val);
     string init = b.to_string();
     int count = 1;
     for(int i = 0;i<plain.size();i=i+16){
          string temp = plain.substr(i,16);
          string z ="";
          for(int j = 0;j<init.size();j=j+8){
               int sum = 0;
               if(temp[i] == '1')sum+=128;
               if(temp[i+1] == '1')sum+=64;
               if(temp[i+2] == '1')sum+=32;
               if(temp[i+3] == '1')sum+=16;
               if(temp[i+4] == '1')sum+=8;
               if(temp[i+5] == '1')sum+=4;
               if(temp[i+6] == '1')sum+=2;
               if(temp[i+7] == '1')sum+=1;
          ...
          vector<vector<string>> enc = encrypt(z,keys);
          string t = matriceToString(enc);
          string v ="";
          ...
          cout<<"For plain text part "<<count<<endl;count++;
          cout<<"Encrypted Text is "<<bintoHexHelper(XOR(v,t))<<endl;
          count_val ++;
          bitset<128>b(count_val);
          init = b.to_string();
     }
...
int main(){
    ...
    string plain = "TwoOneNineTwoTwoOneNineTwo";
    string res = "";
